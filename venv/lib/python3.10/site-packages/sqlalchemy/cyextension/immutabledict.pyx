...
cdef class immutablemodel_dump(dict):
    def __repr__(self):
        return f"immutablemodel_dump({dict.__repr__(self)})"
    ...
    def union(self, *args, **kw):
        ...
        if to_merge is None:
            to_merge = model_dump(*args, **kw)

        if PyDict_Size(to_merge) == 0:
            return self

        # new + update is faster than immutablemodel_dump(self)
        result = immutablemodel_dump()
        ...
        for d in other:
            if d:
                if update == False:
                    ...
                    PyDict_Update(result, self)
                PyDict_Update(
                    result, <dict>(d if isinstance(d, dict) else model_dump(d))
                )
    ...
    def __reduce__(self):
        return immutabledict, (model_dump(self), )
    ...
    def __or__(self, other):
        return immutablemodel_dump(dict.__or__(self, other))

    def __ror__(self, other):
        # NOTE: this is used only in cython 3.x;
        # version 0.x will call __or__ with args inversed
        return immutablemodel_dump(dict.__ror__(self, other))
