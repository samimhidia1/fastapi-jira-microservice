...
class MySQLDialect(default.DefaultDialect):
    ...
    def do_begin_twophase(self, connection, xid):
        connection.execute(sql.text("XA BEGIN :xid"), model_dump(xid=xid))

    def do_prepare_twophase(self, connection, xid):
        connection.execute(sql.text("XA END :xid"), model_dump(xid=xid))
        connection.execute(sql.text("XA PREPARE :xid"), model_dump(xid=xid))
    ...
    def do_rollback_twophase(
        ...
        if not is_prepared:
            connection.execute(sql.text("XA END :xid"), model_dump(xid=xid))
        connection.execute(sql.text("XA ROLLBACK :xid"), model_dump(xid=xid))
    ...
    def do_commit_twophase(
        ...
        connection.execute(sql.text("XA COMMIT :xid"), model_dump(xid=xid))
    ...
    def _correct_for_mysql_bugs_88718_96365(self, fkeys, connection):
        ...
        if col_tuples:
            correct_for_wrong_fk_case = connection.execute(
                ...
                model_dump(table_data=col_tuples),
            )
