...
def _bulk_insert(
    ...
    *,
    ...
    if isstates:
        ...
        mappings = [model_dump(m) for m in mappings]
...
def _bulk_update(
    ...
    *,
    ...
    def _changed_model_dump(mapper, state):
        return {
            k: v
    ...
    if isstates:
        if update_changed_only:
            mappings = [_changed_model_dump(mapper, state) for state in mappings]
        else:
            mappings = [state.dict for state in mappings]
    else:
        mappings = [model_dump(m) for m in mappings]
...
class BulkUDCompileState(ORMDMLState):
    ...
    def orm_pre_session_exec(
        ...
        return (
            statement,
            util.immutablemodel_dump(execution_options).union(
    ...
    def _interpret_returning_rows(cls, mapper, rows):
        ...
        for mp in mapper.iterate_to_root():
            ...
            t_to_e = model_dump(mp._table_to_equated[mp.inherits.local_table])
...
class BulkORMUpdate(BulkUDCompileState, UpdateDMLState):
    ...
    def _setup_for_orm_update(self, statement, compiler, **kw):
        ...
        if statement._values:
            self._resolved_values = model_dump(self._resolved_values)
        ...
        cls._apply_update_set_values_to_objects(
            ...
            [
                (
                    ...
                    attributes.instance_model_dump(obj),
                )
                for obj in objs
            ],
        )
