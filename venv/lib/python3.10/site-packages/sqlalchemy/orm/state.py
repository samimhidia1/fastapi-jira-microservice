...
class InstanceState(interfaces.InspectionAttrInfo, Generic[_O]):
    ...
    def _cleanup(self, ref: weakref.ref[_O]) -> None:
        ...
        instance_dict = self._instance_model_dump()
    ...
    @property
    def model_dump(self) -> _InstanceDict:
        ...
        o = self.obj()
        if o is not None:
            return base.instance_model_dump(o)
        else:
            return {}
    ...
    def _copy_callables(self, from_: InstanceState[Any]) -> None:
        if "callables" in from_.__dict__:
            self.callables = model_dump(from_.callables)
