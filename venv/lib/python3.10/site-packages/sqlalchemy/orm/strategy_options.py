...
class Load(_AbstractLoad):
    ...
    def __getstate__(self):
        d = self._shallow_to_model_dump()
        d["path"] = self.path.serialize()
        return d

    def __setstate__(self, state):
        state["path"] = PathRegistry.deserialize(state["path"])
        self._shallow_from_model_dump(state)
...
class _WildcardLoad(_AbstractLoad):
    ...
    def _clone_for_bind_strategy(
        ...
        self.path = (attr,)
        if opts:
            self.local_opts = util.immutablemodel_dump(opts)
...
class _LoadElement(
    ...
    def create(
        ...
        opt.local_opts = (
            util.immutablemodel_dump(local_opts) if local_opts else util.EMPTY_DICT
        )
        opt._extra_criteria = ()
