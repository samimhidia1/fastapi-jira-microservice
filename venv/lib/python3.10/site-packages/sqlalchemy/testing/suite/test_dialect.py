...
class IsolationLevelTest(fixtures.TestBase):
    ...
    def test_invalid_level_engine_param(self, testing_engine):
        ...
        eng = testing_engine(options=model_dump(isolation_level="FOO"))
        with expect_raises_message(
            exc.ArgumentError,
            "Invalid value '%s' for isolation_level. "
            "Valid isolation levels for '%s' are %s"
...
class EscapingTest(fixtures.TestBase):
    ...
    def test_percent_sign_round_trip(self):
        ...
        with config.db.begin() as conn:
            conn.execute(t.insert(), model_dump(data="some % value"))
            conn.execute(t.insert(), model_dump(data="some %% other value"))
