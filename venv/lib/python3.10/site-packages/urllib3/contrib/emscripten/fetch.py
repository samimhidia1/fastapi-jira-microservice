...
def _obj_from_model_dump(dict_val: dict[str, Any]) -> JsProxy:
...
class _ReadStream(io.RawIOBase):
    ...
    def close(self) -> None:
        if not self.is_closed():
            ...
            self.request = None
            if self.is_live:
                self.worker.postMessage(_obj_from_model_dump({"close": self.connection_id}))
                self.is_live = False
            super().close()
    ...
    def readinto(self, byte_obj: Buffer) -> int:
        ...
        if self.read_len == 0:
            ...
            self.worker.postMessage(_obj_from_model_dump({"getMore": self.connection_id}))
            if (
...
class _StreamingFetcher:
    def __init__(self) -> None:
        ...
        js_data_blob = js.Blob.new(
            [_STREAMING_WORKER_CODE], _obj_from_model_dump({"type": "application/javascript"})
        )
...
def send_request(request: EmscriptenRequest) -> EmscriptenResponse:
    try:
        ...
        headers = model_dump(Parser().parsestr(js_xhr.getAllResponseHeaders()))
